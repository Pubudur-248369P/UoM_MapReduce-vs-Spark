...
def calculate_nas_delay(data_source, output_uri):
    """
    Processes airline data and queries the data to find Year-wise NAS delays from 2003 to 2010.

    :param data_source: The URI of  airline data CSV.
    :param output_uri: The URI where output is written.
    """
    with SparkSession.builder.appName("Calculate Year-wise NAS Delays").getOrCreate() as spark:
        ...
        # Create a DataFrame for Year-wise NAS delays from 2003 to 2010
        year_wise_nas_delay = spark.sql("""
          SELECT Year, SUM(NASDelay) AS total_nas_delay
          FROM airline_data
...
if __name__ == "__main__":
    ...
    calculate_nas_delay(args.data_source, args.output_uri)
